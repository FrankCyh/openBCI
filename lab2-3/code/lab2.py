...
def plot_eeg_data(data_df, file_name=None):
    ...
    for col_name in data_df.columns.values:
        ...
        if col_name.startswith(EEG_CHANNEL_PREFIX):
            ax[plot_idx].plot(data_df[col_name][500:], color=EEG_CHANNEL_COLORS[col_name[-1]]) # filter out the first 500 data points
            plot_idx += 1
    ...
    if file_name:
        os.chdir(os.path.join(ROOT_DIR, DATA_DIR, "img"))
