...
# import data
folder_path='C:\\Users\\huxia\\Downloads\\motor-imagery-classification'
selected_columns = [3,4,8] # C3 Cz C4 channels are all what we need

X_imagery_LH,y_imagery_LH = data_generator.read_and_select_columns(folder_path + "\\imagery_LH", selected_columns)
X_imagery_RH,y_imagery_RH = data_generator.read_and_select_columns(folder_path + "\\imagery_RH", selected_columns)
...
time_windows = time_windows = np.array([[1.5,2.5],
		 				  [2,3],
		 			   	[2.5,3.5],
		 				  [3,4],
		 				  [3.5,4.5],
...
CNN_model.save(folder_path+'/model_init/model_motor.h5'.format())
CNN_model = load_model(folder_path+'\\model_init\\model_motor.h5'.format())
